...
(function () {
    ...
    registerSubmitBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (regRes && regRes.auto_verified && regRes.token) {
          ...
          showToast(regRes.message || "تم إنشاء الحساب بنجاح! مرحباً بك ðŸŽ‰", "success");
  ...
  function goToScreen(name, pushState = true) {
    ...
    switch (name) {
      ...
      case "add": checkAndPrepareCouponsForAdd(); break;
  ...
  async function performSave() {
    ...
    try {
      ...
      if (document.getElementById("useCouponCheckbox")?.checked && selectedAdCouponId && !state.currentEditAdId) {
        formData.append("coupon_id", selectedAdCouponId);
      }
      ...
      if (token && state.user?.id) {
        ...
        try {
          ...
          if (!patchRes.ok) {
            ...
            showFormError(errEl, errData?.username?.[0] || errData?.error || (state.lang === 'ar' ? 'فشل حفظ التعديلات' : 'Failed to save changes'));
          ...
          showFormError(errEl, state.lang === 'ar' ? 'خطأ في الاتصال بالسيرفر' : 'Server connection error');
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('submit', async function(e) {
      ...
      const originalText = btn?.innerText || 'إرسال';

      if (btn) { btn.disabled = true; btn.innerText = state.lang === 'ar' ? 'جاري الإرسال...' : 'Sending...'; }
      ...
      try {
        ...
        if (response.ok) {
          ...
          showToast(state.lang === 'ar' ? 'تم إرسال رسالتك بنجاح ✅' : 'Message sent successfully ✅', 'success');
          ...
          showToast(errData?.message || (state.lang === 'ar' ? 'فشل إرسال الرسالة' : 'Failed to send message'), 'error');

        }

      } catch (error) {

        showToast(state.lang === 'ar' ? 'خطأ في الاتصال بالسيرفر' : 'Server connection error', 'error');
  ...
  // ADMIN GIFT COUPONS LOGIC

  // ================================================

  const grantTargetRadios = document.querySelectorAll('input[name="grantTarget"]');

  const specificDiv = document.getElementById('grantSpecificUserDiv');

  

  if(grantTargetRadios.length) {

    grantTargetRadios.forEach(r => r.addEventListener('change', (e) => {
  ...
  const grantSearchBtn = document.getElementById('grantSearchBtn');

  if(grantSearchBtn) {

    grantSearchBtn.addEventListener('click', async () => {

      const query = document.getElementById('grantSearchInput').value.trim();
      ...
      try {
        ...
        const res = await fetch(BASE_URL + '/users/?search=' + encodeURIComponent(query), {
          ...
          headers: { 'Authorization': 'Token ' + localStorage.getItem('dj_token') }
        ...
        if(Array.isArray(users) && users.length) {
          ...
          select.innerHTML = '<option value="">لا توجد نتائج</option>';

        }

      } catch(e) {

        select.innerHTML = '<option value="">خطأ في البحث</option>';
  ...
  if(grantSubmitBtn) {
    ...
    grantSubmitBtn.addEventListener('click', async () => {
      ...
      const target = document.querySelector('input[name="grantTarget"]:checked').value;

      const count = document.getElementById('grantCountInput').value;
      ...
      if (target === 'specific') {
        ...
        userId = document.getElementById('grantUserSelect').value;
      ...
      const msgDiv = document.getElementById('grantResultMsg');

      msgDiv.innerText = 'جاري الإرسال...';

      msgDiv.style.color = '#FF6A00';

      

      try {

        const res = await fetch(BASE_URL + '/admin/grant-coupons/', {
          ...
          body: JSON.stringify({ target: target, user_id: userId, count: parseInt(count) })
        ...
        if(res.ok) {
          ...
          msgDiv.innerText = `تم إرسال ${count} قسيمة لـ ${data.users_count} مستخدم بنجاح`;

          msgDiv.style.color = '#10b981';

        } else {

          msgDiv.innerText = 'خطأ: ' + (data.error || 'غير معروف');
